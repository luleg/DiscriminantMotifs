% script to create the distance-to-mean classifier based on the LDA of the
...
disp_proj = 1; % do you want to display the Figures ?
...
for iter = 1:nb_ites
    ...
    % calling of the lda script, that manages the LDAprocess
    lda
...
if disp_proj
    ...
    sgtitle("Projection of the training set")
    zz= zeros(nb_basis,1);
    for ii=1:nb_ax
        subplot(nb_ax,1,ii)
        plot([min(C(:,ii))-1e-8,max(C(:,ii))+1e-8,],[0,0],'k-',C(fw,ii),zz,'r+',C(elec,ii),zz,'bo',C(stac,ii),zz,'k+',C(soc,ii),zz,'mo')
    end
    legend('','FoodWebs','Elec.','Disc','Soc')
    
    
    figure(2), clf,hold on
    sgtitle("Projection of the test set")
    zzfw= zeros(nbt_fw,1);
    zzelec= zeros(nbt_elec,1);
    zzstac= zeros(nbt_stac,1);
    zzsoc= zeros(nbt_soc,1);
    for ii=1:nb_ax
        subplot(nb_ax,1,ii)
        plot([min(Ct(:,ii))-1e-8,max(Ct(:,ii))+1e-8,],[0,0],'k-',Ct(fwt,ii),zzfw,'r+',Ct(elect,ii),zzelec,'bo',Ct(stact,ii),zzstac,'k+',Ct(soct,ii),zzsoc,'mo')
    end
    legend('','FoodWebs','Elec.','Disc','Soc')
