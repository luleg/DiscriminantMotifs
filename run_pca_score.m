% script to create the k-nn classifier based on the PCA of the networks
% decomposed onto 3-node and 4-node motifs.
clear variables;clc;
close all
...
for iter = 1:nb_ites
    ...
    Xt = Vt-x_b*ones(1,nbt_reseaux); % change of origin
    
    Ct = Xt'*U(:,1:nb_ax); % change of axes
    knn
    ...
    if disp_fig
        disp_dataset(C_all,fw_all,elec_all,stac_all,soc_all,7)
    end
    
    tens_infos(:,:,iter) = infos;
    tens_Conf(:,:,iter) = Conf;
end

[tab_prec,tab_rec,tab_f1,Conf] = postproc_scores(tens_infos,tens_Conf);
