...
tens_infos = zeros(6,4,nb_ites,nb_tests);
...
% => Here, we do it for each number of kept  motifs

tens_Conf = zeros(4,4,nb_ites,nb_tests);
...
f1_all = zeros(nb_tests,4);
std_all = zeros(nb_tests,4);
Conf_all = zeros(4,4,nb_tests);
for nb_motifs=1:30
    [tab_prec,tab_rec,tab_f1,Conf] = postproc_scores(tens_infos(:,:,:,nb_motifs),tens_Conf(:,:,:,nb_motifs));
    f1_all(nb_motifs,:) = tab_f1(3,:);
    std_all(nb_motifs,:) = tab_f1(4,:);
    Conf_all(:,:,nb_motifs) = Conf;
end
